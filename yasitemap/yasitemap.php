...
class Yasitemap extends Module
{
	...
	public function __construct()
	{
		...
		$this->displayName = $this->l('Yandex sitemap');
		$this->description = $this->l('Generate your Yandex sitemap file');
	}

	/**
	 * Yandex Sitemap installation process:
	 *
	...
	{
		foreach (array('YASITEMAP_PRIORITY_HOME' => 1.0, 'YASITEMAP_PRIORITY_PRODUCT' => 0.9, 'YASITEMAP_PRIORITY_CATEGORY' => 0.8,
	'YASITEMAP_PRIORITY_MANUFACTURER' => 0.7, 'YASITEMAP_PRIORITY_SUPPLIER' => 0.6, 'YASITEMAP_PRIORITY_CMS' => 0.5,
	'YASITEMAP_FREQUENCY' => 'weekly', 'YASITEMAP_CHECK_IMAGE_FILE' => false, 'YASITEMAP_LAST_EXPORT' => false) as $key => $val)
			if (!Configuration::updateValue($key, $val))
				return false;
		return parent::install() && Db::getInstance()->Execute('CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'yasitemap_sitemap` (`link` varchar(255) DEFAULT NULL, `id_shop` int(11) DEFAULT 0) ENGINE='._MYSQL_ENGINE_.' DEFAULT CHARSET=utf8;');
	}

	/**
	 * Yandex Sitemap uninstallation process:
	 *
	...
	{
		foreach (array('YASITEMAP_PRIORITY_HOME' => '', 'YASITEMAP_PRIORITY_PRODUCT' => '', 'YASITEMAP_PRIORITY_CATEGORY' => '',
	'YASITEMAP_PRIORITY_MANUFACTURER' => '', 'YASITEMAP_PRIORITY_SUPPLIER' => '', 'YASITEMAP_PRIORITY_CMS' => '',
	'YASITEMAP_FREQUENCY' => '', 'YASITEMAP_CHECK_IMAGE_FILE' => '', 'YASITEMAP_LAST_EXPORT' => '') as $key => $val)
	...
	{
		$links = Db::getInstance()->ExecuteS('SELECT * FROM `'._DB_PREFIX_.'yasitemap_sitemap`');
		if ($links)
			foreach ($links as $link)
				if (!@unlink(dirname(__FILE__).'/../../'.$link['link']))
					return false;
		if (!Db::getInstance()->Execute('DROP TABLE `'._DB_PREFIX_.'yasitemap_sitemap`'))
			return false;
		return true;
	}
	...
	{
		/* Store the posted parameters and generate a new Yandex Sitemap files for the current Shop */
		if (Tools::isSubmit('SubmitYasitemap'))
		{
			Configuration::updateValue('YASITEMAP_FREQUENCY', pSQL(Tools::getValue('yasitemap_frequency')));
			Configuration::updateValue('YASITEMAP_INDEX_CHECK', '');
			Configuration::updateValue('YASITEMAP_CHECK_IMAGE_FILE', pSQL(Tools::getValue('yasitemap_check_image_file')));
			$meta = '';
			if (Tools::getValue('yasitemap_meta'))
				$meta .= implode(', ', Tools::getValue('yasitemap_meta'));
			Configuration::updateValue('YASITEMAP_DISABLE_LINKS', $meta);
			$this->emptySitemap();
			$this->createSitemap();
		}
		...
		if (_PS_VERSION_ < 1.5)
			require(_PS_MODULE_DIR_.'yasitemap/backward_compatibility/backward.php');
		...
		$this->context->smarty->assign(array(
			'yasitemap_form' => './index.php?tab=AdminModules&configure=yasitemap&token='.Tools::getAdminTokenLite('AdminModules').'&tab_module='.$this->tab.'&module_name=yasitemap',
			'yasitemap_cron' => _PS_BASE_URL_._MODULE_DIR_.'yasitemap/yasitemap-cron.php?token='.substr(Tools::encrypt('yasitemap/cron'), 0, 10).'&id_shop='.$this->context->shop->id,
			'yasitemap_feed_exists' => file_exists(dirname(__FILE__).'/../../index_yasitemap.xml'),
			'yasitemap_last_export' => Configuration::get('YASITEMAP_LAST_EXPORT'),
			'yasitemap_frequency' => Configuration::get('YASITEMAP_FREQUENCY'),
			'yasitemap_store_url' => 'http'.(Configuration::get('PS_SSL_ENABLED') ? 's' : '').'://'.Tools::getShopDomain(false, true).__PS_BASE_URI__,
			'yasitemap_links' => Db::getInstance()->ExecuteS('SELECT * FROM `'._DB_PREFIX_.'yasitemap_sitemap` WHERE id_shop = '.(int)$this->context->shop->id),
			'store_metas' => Meta::getMetasByIdLang((int)$this->context->cookie->id_lang),
			'yasitemap_disable_metas' => explode(',', Configuration::get('YASITEMAP_DISABLE_LINKS')),
			'yasitemap_customer_limit' => array('max_exec_time' => (int)ini_get('max_execution_time'), 'memory_limit' => intval(ini_get('memory_limit'))),
			'prestashop_version' => _PS_VERSION_ >= 1.5 ? '1.5' : '1.4',
			'prestashop_ssl' => Configuration::get('PS_SSL_ENABLED'),
			'yasitemap_check_image_file' => Configuration::get('YASITEMAP_CHECK_IMAGE_FILE'),
	...
	public function emptySitemap($id_shop = 0)
	{
		...
		$links = Db::getInstance()->ExecuteS('SELECT * FROM `'._DB_PREFIX_.'yasitemap_sitemap` WHERE id_shop = '.(int)$this->context->shop->id);
		if ($links)
		{
			foreach ($links as $link)
				@unlink(dirname(__FILE__).'/../../'.$link['link']);
			return Db::getInstance()->Execute('DELETE FROM `'._DB_PREFIX_.'yasitemap_sitemap` WHERE id_shop = '.(int)$this->context->shop->id);
		}
		return true;
	}
	...
	public function _addLinkToSitemap(&$link_sitemap, $new_link, $lang, &$index, &$i, $id_obj)
	{
		...
		else
		{
			...
			{
				$this->context->smarty->assign(array(
					'yasitemap_number' => (int)$index,
					'yasitemap_refresh_page' => './index.php?tab=AdminModules&configure=yasitemap&token='.Tools::getAdminTokenLite('AdminModules').'&tab_module='.$this->tab.'&module_name=yasitemap&continue=1&type='.$new_link['type'].'&lang='.$lang.'&index='.$index.'&id='.($id_obj + 1).'&id_shop='.$this->context->shop->id));
				return false;
			}
			else if ($index % 20 == 0 && $this->cron)
			{
				header('Refresh: 5; url=http'.(Configuration::get('PS_SSL_ENABLED') ? 's' : '').'://'.Tools::getShopDomain(false, true).__PS_BASE_URI__.'modules/yasitemap/yasitemap-cron.php?continue=1&token='.substr(Tools::encrypt('yasitemap/cron'), 0, 10).'&type='.$new_link['type'].'&lang='.$lang.'&index='.$index.'&id='.($id_obj + 1).'&id_shop='.$this->context->shop->id);
				die();
			}
			else
			{
				if ($this->cron)
					header('location: http'.(Configuration::get('PS_SSL_ENABLED') ? 's' : '').'://'.Tools::getShopDomain(false, true).__PS_BASE_URI__.'modules/yasitemap/yasitemap-cron.php?continue=1&token='.substr(Tools::encrypt('yasitemap/cron'), 0, 10).'&type='.$new_link['type'].'&lang='.$lang.'&index='.$index.'&id='.($id_obj + 1).'&id_shop='.$this->context->shop->id);
				else
				{
					$admin_folder = str_replace(_PS_ROOT_DIR_, '', _PS_ADMIN_DIR_);
					$admin_folder = substr($admin_folder, 1);
					header('location: http'.(Configuration::get('PS_SSL_ENABLED') ? 's' : '').'://'.Tools::getShopDomain(false, true).__PS_BASE_URI__.$admin_folder.'/index.php?tab=AdminModules&configure=yasitemap&token='.Tools::getAdminTokenLite('AdminModules').'&tab_module='.$this->tab.'&module_name=yasitemap&continue=1&type='.$new_link['type'].'&lang='.$lang.'&index='.$index.'&id='.($id_obj + 1).'&id_shop='.$this->context->shop->id);
				}
				die();
			}
		}
	...
	private function _getMetaLink(&$link_sitemap, $lang, &$index, &$i, $id_meta = 0)
	{
		...
		{
			$url = '';
			if (!in_array($meta['id_meta'], explode(',', Configuration::get('YASITEMAP_DISABLE_LINKS'))))
			{
				if (_PS_VERSION_ >= 1.5)
				{
...
/*			$id_image = Product::getCover((int)$product_id['id_product']);
			...
			$file_headers = (Configuration::get('YASITEMAP_CHECK_IMAGE_FILE')) ? @get_headers($image_link) : true;
			$image_product = array();
...
//			if (!$this->_addLinkToSitemap($link_sitemap, array('type' => 'supplier', 'page' => 'supplier', 'link' => $url, 'image' => $supplier_image), $lang['iso_code'], $index, $i, $supplier_id['id_supplier']))
	...
	public function createSitemap($id_shop = 0)
	{
		...
		Configuration::updateValue('YASITEMAP_LAST_EXPORT', date('r'));
		...
		if ($this->cron)
			return true;
		header('location: ./index.php?tab=AdminModules&configure=yasitemap&token='.Tools::getAdminTokenLite('AdminModules').'&tab_module='.$this->tab.'&module_name=yasitemap&validation');
		die();
	}
	...
	{
		if ($sitemap)
			return Db::getInstance()->Execute('INSERT INTO `'._DB_PREFIX_.'yasitemap_sitemap` (`link`, id_shop) VALUES (\''.pSQL($sitemap).'\', '.(int)$this->context->shop->id.')');
	}
	...
	private function _recursiveSitemapCreator($link_sitemap, $lang, &$index)
	{
		...
		$sitemap_link = $this->context->shop->id.'_'.$lang.'_'.$index.'_yasitemap.xml';
	...
	{
		return Configuration::get('YASITEMAP_PRIORITY_'.Tools::strtoupper($page)) ? Configuration::get('YASITEMAP_PRIORITY_'.Tools::strtoupper($page)) : 0.1;
	}
	...
	{
		$sitemaps = Db::getInstance()->ExecuteS('SELECT `link` FROM `'._DB_PREFIX_.'yasitemap_sitemap` WHERE id_shop = '.$this->context->shop->id);
		if (!$sitemaps)
			return false;
		...
		file_put_contents(dirname(__FILE__).'/../../'.$this->context->shop->id.'_index_yasitemap.xml', $xml_feed->asXML());
	}
