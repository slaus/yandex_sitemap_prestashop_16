...
/*
 * This file can be called using a cron to generate Yandex Sitemap files automatically
 */

include(dirname(__FILE__).'/../../config/config.inc.php');
include(dirname(__FILE__).'/../../init.php');
/* Check to security tocken */
if (substr(Tools::encrypt('yasitemap/cron'), 0, 10) != Tools::getValue('token') || !Module::isInstalled('yasitemap'))
	die('Bad token');

include(dirname(__FILE__).'/yasitemap.php');
$yasitemap = new Yasitemap();
...
if ($yasitemap->active)
{
	...
	if (!isset($_GET['continue']))
		$yasitemap->emptySitemap((int)$id_shop);

	/* Create the Yandex Sitemap's files */
	p($yasitemap->createSitemap((int)$id_shop));
